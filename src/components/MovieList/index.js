...
export class MovieList extends Component {
  addResults = async () => {
    ...
    const moreMovies = await this.props.getMovies(page + 1);
  ...
  render() {
    ...
    const displayedMovies = movies.map((movie, index) => {
      const starColor = favorites.includes(movie.movie_id)
        ? '#ffd24d'
        : '#f2f2f2';
    ...
    return (
      <main>
        <div className="movie-cards">{displayedMovies}</div>
        <button className="more-btn" onClick={() => this.addResults()}>
          View More
        </button>
      </main>
    );
